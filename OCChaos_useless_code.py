...
def getNameArray():
    for i in range(500):
        ...
        name_array.append(final)


# .h文件添加废代码

def HFileAddMj(file_path, old_str_arr):
    file_data = ""
    ...
    for line in Ropen:
        ...
        nameStr = random.choice(name_array)

        className = random.choice(OCChaos_config.classArray)
        ...
        for old_str in old_str_arr:
            ...
            if old_str in line:
                line += '\n\n/*********FQ代码**********/\n' \
                        ...
                        '\n/*********FQ代码**********/\n\n'
                name_array.remove(nameStr)  # 防止创建的属性名重复(创建一个从数组中删除一个)
                break;
...
def MFileAddMj(file_path, old_str_arr):
    file_data = ""
    ...
    for line in Ropen:
        ...
        for old_str in old_str_arr:
            ...
            if old_str in line:  # 如果.h文件中的某一行里包含old_str,则往这一行添加一下语句
                ...
                name_array.remove(nameStr)  # 防止创建的元素名重复(创建一个从数组中删除一个)
                break;
        file_data += line
    ...
    for root, dirs, files in os.walk(file_dir):
        ...
        for file in files:
            ...
            if '.h' in file:  # file_dir+'/'+file含义是file_dir文件夹下的file文件
                ...
                HFileAddMj(file_dir + '/' + file, OCChaos_config.h_file_mark_arr)

            if '.m' in file:
                MFileAddMj(file_dir + '/' + file, OCChaos_config.m_file_mark_arr)


def start_create_useless_code():
    getNameArray()
    # 要修改的文件所在的文件夹路径
    file_name(OCChaos_config.file_floadPath)


if __name__ == "__main__":
    start_create_useless_code()
