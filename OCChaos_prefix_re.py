...
def file_rename(file_path):
    ...
    new_path = os.path.join(root_path,
                            filename.replace(OCChaos_config.pre_str, OCChaos_config.pre_to_str) + filetype)  # 拼接新路径
    os.renames(file_path, new_path)  # 文件重命名
    return filename.replace(OCChaos_config.pre_str, OCChaos_config.pre_to_str)
...
def start_rename():
    ...
    for (root, dirs, files) in os.walk(OCChaos_config.project_path):
        for file_name in files:
            if file_name.startswith((OCChaos_config.pre_str,)) and file_name.endswith(OCChaos_config.suf_set):
    ...
    for (root, dirs, files) in os.walk(OCChaos_config.project_path):
        for file_name in files:
            if file_name.endswith(OCChaos_config.suf_set):
    ...
    # 替换配置文件中的类名
    for key in needModifyDic:
        with open(OCChaos_config.pbxpro_path, 'r+') as f:
            s0 = f.read()
            f.close()
            if key in s0:
                with open(OCChaos_config.pbxpro_path, 'r+') as f2:
